...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, reciever = 'mei30', amount = 1    )
...
def get_chain():
    ...
    if not all (key in json for key in transaction_keys):
        return 'Some elemnts of the transaction are missing', 400
...
def connect_node():
    ...
    response = {'message': 'All the nodes now are connected.',
                'total_connected_nodes': list(blockchain.nodes)}    
    return jsonify(response), 201
